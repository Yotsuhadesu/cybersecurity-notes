...
- `sleep [seconds]` - a process that puts itself to sleep the duration
- `PID` - process ID
...
## Technicalities
- `/etc/hosts`
  - A file where the OS resolver checks first when you enter a URL.
  - If the domain name is found, it will immediately use the IP address beside it. 
- `/etc/resolv.conf`
  - A file where the OS checks next if the domain name isn't on `/etc/hosts`.
...
- Close or block unfamiliar/unused ports.  
