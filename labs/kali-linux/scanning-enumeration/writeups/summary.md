...
- `nmap -sV --version-intensity [0-9] [IP Address]` - adjusts how thoroughly Nmap does the version scan
...
### Technicalities
- Use a full TCP scan to scan if the target is alive and determine which ports it listens to.
- Use a service/version scan to identify the services and their versions, which can be used to search for CVEs.
...
- I didn't expect that all 256 hosts of the scanme.nmap.org network are up.
