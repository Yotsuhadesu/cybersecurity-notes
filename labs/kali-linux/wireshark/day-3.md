...
DNS Lookup Process:
1. The browser asks the DNS resolver (such as a router or default gateway) for the HTTPS, A (IPv4), and AAAA (IPv6) records of the website server.
...
3. The browser sends an ACK flag, establishing the connection.
...
Thoughts:
- SNI (Server Name Indicator) is showing the website's domain name.
- In theory, the process should be like this:
  ...
  4. They exchange encryption keys.
  5. Encrypted communication begins.
- However, on TLS version 1.3, the certificate is encrypted - appears as "Application Data"
...
- The rest of the content doesn't form a proper word/phrase.
...
| Visible Contents | All | Domain Name, IPs, time |
| Port | 80 | 443|
| Can infer sites visited? | YES | YES (DNS and SNI) |
...
- The repeated "ACK" flags signal that the data has been received.
